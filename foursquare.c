...
foursquare_perform_query ()
{
  ...
  snprintf(url, URL_SIZE, URL_FORMAT, coords, categoryId, client_id, client_secret, api_version);
...
for(i = 0; i < json_array_size(venues); i++)
{
  ...
  name = json_object_get(venue, "name");
    ...
    {
        fprintf(stderr, "error: venue.name %d: sha is not a string\n", i + 1);
        json_decref(root);
        return 1;
    } 
  location = json_object_get(venue, "location");
    ...
    {
        fprintf(stderr, "error: venue.location %d: sha is not an object\n", i + 1);
        json_decref(root);
        return 1;
    } 
	printf("Found %s at %i:)\n",json_string_value(name), json_integer_value(json_object_get(location, "distance")));
    }
   }
json_decref(root);
  }
  ...
  if (add)
    {
      ...
      callback.u.callback =
	callback_new_attr_0 (callback_cast (foursquare_navit_init), attr_navit);
